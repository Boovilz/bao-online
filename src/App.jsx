...
const DEFAULT_PLAYER = {
  ...
  learning: { scienceCompleted: false, scienceBest: 0 },
  badges: [],
};

const ITEM_INFO = {
  seedling: { icon: '🌱', name: 'ต้นกล้า' }, flower: { icon: '🌼', name: 'ดอกไม้' },
  book: { icon: '📚', name: 'หนังสือ' }, lego: { icon: '🧱', name: 'ชิ้นส่วน LEGO' },
};
...
function loadPlayer() {
  try { return normalizePlayer(JSON.parse(localStorage.getItem('bao-player') || '{}')); }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    gameRef.current = createGame('game', {
      player,
      onPlace: (place) => {
        if (place.name === 'ห้องวิทยาศาสตร์') setPanel({ type: 'science-intro', ...place });
        else setPanel({ type: 'place', ...place });
      },
  ...
  const login = (event) => { event.preventDefault(); if (!username.trim()) return; setPlayer((p) => ({ ...p, username: username.trim() })); setScreen('character'); };
  const chooseCharacter = (character) => { setPlayer((p) => ({ ...p, character, name: character === 'girl' ? 'น้องข้าวหอม' : 'น้องต้นกล้า' })); setScreen('world'); };
  ...
  const addExp = (current, amount) => {
    const total = current.exp + amount;
    return { level: current.level + Math.floor(total / 100), exp: total % 100 };
  };

  const claimReward = () => {
    setPlayer((current) => {
      if (!current.quest.completed || current.quest.claimed) return current;
      const progress = addExp(current, 120);
      return { ...current, ...progress, coin: current.coin + 50, quest: { ...current.quest, claimed: true } };
    });
    setPanel({ type: 'reward' });
  };

  const startScience = () => { setQuiz({ index: 0, score: 0, answered: false, selected: null }); setPanel({ type: 'science-quiz' }); };
  const answerScience = (choice) => {
    if (quiz.answered) return;
    const correct = choice === SCIENCE_QUIZ[quiz.index].answer;
    setQuiz((q) => ({ ...q, answered: true, selected: choice, score: q.score + (correct ? 1 : 0) }));
  };
  const nextScience = () => {
    if (quiz.index < SCIENCE_QUIZ.length - 1) {
      setQuiz((q) => ({ ...q, index: q.index + 1, answered: false, selected: null }));
      return;
    }
    const finalScore = quiz.score;
    setPlayer((current) => {
      const firstClear = !current.learning.scienceCompleted;
      const progress = firstClear ? addExp(current, 80) : { level: current.level, exp: current.exp };
      return {
        ...current, ...progress,
        coin: current.coin + (firstClear ? 40 : 0),
        inventory: { ...current.inventory, book: current.inventory.book + (firstClear ? 1 : 0) },
        learning: { ...current.learning, scienceCompleted: true, scienceBest: Math.max(current.learning.scienceBest, finalScore) },
        badges: firstClear && !current.badges.includes('Young Scientist') ? [...current.badges, 'Young Scientist'] : current.badges,
      };
    });
    setPanel({ type: 'science-result', score: finalScore });
  };

  if (screen === 'login') return <main className="splash login-screen"><div className="sky-cloud cloud-one" /><div className="sky-cloud cloud-two" /><section className="login-card game-panel"><div className="brand-lockup"><div className="logo">BAO <span>ONLINE</span></div><div className="leaf-mark">🌱</div></div><h1>บ้านเฮาออนไลน์</h1><p className="subtitle">โรงเรียนบ้านเหล่าพ่อหา · โลกแห่งการเรียนรู้</p><form onSubmit={login}><label htmlFor="username">ชื่อผู้เล่น</label><div className="input-frame"><span>👤</span><input id="username" value={username} onChange={(e) => setUsername(e.target.value)} maxLength={24} /></div><button className="primary start-button" type="submit">เข้าสู่บ้านเฮา</button></form><small>V0.5 · Education Platform</small></section></main>;
  ...
  return <main className="app">
    ...
    {panel && <div className="overlay" onClick={() => setPanel(null)}><section className={`dialog ${panel.type === 'science-quiz' ? 'learning-dialog' : ''}`} onClick={(e) => e.stopPropagation()}><button className="close" onClick={() => setPanel(null)}>×</button>
      ...
      {panel.type === 'badges' && <><div className="place-icon">🏅</div><h2>เหรียญตราของฉัน</h2>{player.badges.length ? <div className="badge-card">🔬 <strong>{player.badges.join(', ')}</strong></div> : <p>ยังไม่มีเหรียญตรา ลองเข้าเรียนในห้องวิทยาศาสตร์ดูนะ</p>}</>}
      {panel.type === 'npc' && <><img className="dialog-avatar" src="/assets/npc-teacher.svg" alt="ครูภูมิปัญญา" /><h2>ครูภูมิปัญญา</h2>{!player.quest.accepted && <><p>ช่วยครูเก็บต้นกล้าให้ครบ 3 ต้น แล้วกลับมารับรางวัลนะ</p><button className="primary" onClick={acceptQuest}>รับภารกิจ</button></>}{player.quest.accepted && !player.quest.completed && <p>ตอนนี้เก็บได้ {player.quest.progress}/{player.quest.target} ต้น</p>}{player.quest.completed && !player.quest.claimed && <><p>พร้อมรับ EXP 120 และเหรียญ 50</p><button className="primary" onClick={claimReward}>รับรางวัล</button></>}{player.quest.claimed && <p>ไปลองเรียนที่ห้องวิทยาศาสตร์ได้เลย</p>}</>}
      {panel.type === 'science-intro' && <><div className="place-icon">🧪</div><h2>ห้องวิทยาศาสตร์</h2><p>ภารกิจการเรียนรู้: นักแยกสารตัวน้อย<br />ตอบคำถาม 3 ข้อเกี่ยวกับการแยกสาร</p><div className="lesson-reward">รางวัลครั้งแรก: +80 EXP · 🪙 40 · 📚 1 · Badge</div><button className="primary" onClick={startScience}>เริ่มบทเรียน</button></>}
      {panel.type === 'science-quiz' && <><div className="lesson-progress">ข้อ {quiz.index + 1}/{SCIENCE_QUIZ.length} · คะแนน {quiz.score}</div><div className="place-icon">🧪</div><h2>{question.question}</h2><div className="quiz-choices">{question.choices.map((choice, index) => <button key={choice} disabled={quiz.answered} className={quiz.answered ? (index === question.answer ? 'correct' : index === quiz.selected ? 'wrong' : '') : ''} onClick={() => answerScience(index)}>{choice}</button>)}</div>{quiz.answered && <><p className="feedback">{quiz.selected === question.answer ? '✅ ถูกต้อง เก่งมาก!' : '💡 ลองสังเกตคุณสมบัติของสารอีกครั้ง'}</p><button className="primary" onClick={nextScience}>{quiz.index === SCIENCE_QUIZ.length - 1 ? 'ดูผลการเรียน' : 'ข้อต่อไป'}</button></>}</>}
      {panel.type === 'science-result' && <><div className="place-icon">🏆</div><h2>จบบทเรียนแล้ว!</h2><p>ได้คะแนน {panel.score}/{SCIENCE_QUIZ.length}</p>{!player.learning.scienceCompleted ? null : <div className="badge-card">🔬 Young Scientist</div>}<button className="primary" onClick={() => setPanel(null)}>กลับสู่หมู่บ้าน</button></>}
      {panel.type === 'place' && <><div className="place-icon">{panel.icon}</div><h2>{panel.name}</h2><p>{panel.description}</p><div className="coming-soon">กำลังเตรียมบทเรียนและมินิเกมสำหรับห้องนี้</div><button className="primary" onClick={() => setPanel(null)}>กลับเข้าเมือง</button></>}
