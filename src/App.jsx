...
export default function App() {
  ...
  const postCommunity = (message) => {
    setFeed((items) => [{ id: Date.now(), name: player.name, message }, ...items].slice(0, 5));
    showToast(`${player.name}: ${message}`);
  };
  ...
  useEffect(() => {
    ...
    gameRef.current = createGame('game', {
      player,
      onPlace: (place) => setPanel(place.name === 'ห้องวิทยาศาสตร์' ? { type: 'science-intro', ...place } : { type: 'place', ...place }),
  ...
  const claimReward = () => {
    setPlayer((current) => {
      ...
      return { ...current, ...addExp(current, 120), coin: current.coin + 50, quest: { ...current.quest, claimed: true } };
    });
  ...
  const saveProfile = (event) => {
    ...
    setPlayer((p) => ({ ...p, classroom: data.get('classroom'), number: data.get('number'), team: data.get('team') }));
    setPanel(null); showToast('บันทึกโปรไฟล์ชุมชนแล้ว');
  };
  ...
  const nextScience = () => {
    ...
    setPlayer((current) => {
      ...
      return {
        ...current, ...(firstClear ? addExp(current, 80) : { level: current.level, exp: current.exp }),
  ...
  return <main className="app">
    ...
    <footer className="help">Phase 6 ทดลองระบบชุมชนในเครื่อง · ระบบออนไลน์จริงจะเชื่อมฐานข้อมูลใน Sprint ถัดไป</footer>
